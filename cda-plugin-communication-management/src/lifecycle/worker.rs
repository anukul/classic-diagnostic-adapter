...
use cda_interfaces::communication_control::{CommunicationLifecycle, TransportControl};
...
pub(crate) enum LifecycleCommand {
    ...
    /// is cancelled before the command enters the mailbox.
    ReleaseDisableLease {
        id: DisableLeaseId,
        reply: ActivationReply,
    },
    /// Appends a lifecycle hook to the worker-owned registration list.
    /// Ordering is exact: any `Activate`/`Detect` processed after this command
    /// includes it; anything processed earlier in the queue does not.
...
impl WorkerSender {
    ...
    ///
    /// This is a **terminal** operation: after it returns the worker task has
    /// exited, its channels are closed, and `admission_closed` remains `true`
    /// permanently. The paired `CommunicationStateData::shutting_down` flag
    /// (set by the caller before invoking this method) likewise stays `true`
    /// so that racing detached tasks cannot overwrite the terminal state.
    ///
    ...
    /// oneshot (the worker replies to every queued shutdown request). If the
    /// worker has already exited, returns immediately.
...
impl LifecycleWorker {
    ...
    async fn execute_disable(
        ...
        // registration order (symmetric to initialize's forward order).
        for initializer in self.resources.initializers.iter().rev() {
