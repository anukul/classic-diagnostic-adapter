...
use cda_interfaces::communication_control::{
    CommunicationInitMode, CommunicationLifecycle, TransportControl,
};
...
impl CommunicationHandle {
    ...
    pub async fn register_lifecycle_hook(&self, initializer: Arc<dyn CommunicationLifecycle>) {
        let (tx, rx) = oneshot::channel();
        if self
            .worker
            .send(LifecycleCommand::RegisterLifecycleHook {
                initializer,
                reply: tx,
            })
            .await
    ...
    ///
    /// **Shutdown is terminal and irreversible.** After this method returns
    /// the handle is permanently inert:
    ///
    /// - `activate`, `trigger_detection`, and `disable` return
    ///   `Err(ShuttingDown)` immediately.
    /// - `request_activate` returns `CommunicationState::Error(ShuttingDown)`.
    /// - `register_lifecycle_hook` silently no-ops (the hook is dropped).
    /// - Dropping a held `DisableLease` is a no-op.
    /// - `state()` returns the terminal state the worker published
    ///   (`Disabled`, or `Error` if the transport failed to disable).
    /// - Calling `shutdown` again is idempotent and returns immediately.
    ///
    /// To resume communication after shutdown, construct a new
    /// `CommunicationHandle` with a fresh worker (the runtime-reload
    /// system does this).
...
mod tests {
    ...
    #[allow(
        dead_code,
        reason = "May be used by future tests or downstream consumers"
    )]
    struct TestPlugin {
    ...
    impl CommunicationPlugin for TestPlugin {
        ...
        async fn register_lifecycle_hook(&self, initializer: Arc<dyn CommunicationLifecycle>) {
    ...
    async fn release_cancelled_before_worker_accepts_defers_synchronously() {
        ...
        // Saturate the worker mailbox with RegisterLifecycleHook commands that
    ...
    async fn admission_genuinely_blocks_at_capacity_not_just_eventually_completes() {
        ...
        // draining it.
        let (reply, _rx) = oneshot::channel();
        let ninth = handle.worker.send(LifecycleCommand::RegisterLifecycleHook {
            initializer: Arc::new(CountingInitializer),
            reply,
        });
        tokio::pin!(ninth);
    ...
    #[async_trait]
    impl CommunicationLifecycle for FailingInitializer {
        fn name(&self) -> &'static str {
            "failing"
        }
        async fn initialize(&self) -> Result<(), CommControlError> {
    ...
    #[async_trait]
    impl CommunicationLifecycle for BlockingInitializer {
        fn name(&self) -> &'static str {
            "blocking"
        }
        async fn initialize(&self) -> Result<(), CommControlError> {
