...
impl<S: EcuGateway + FunctionalTransport, T: EcuManager> UdsQuery for UdsManager<S, T> {
    ...
    async fn get_sdgs(
        ...
        self.uds_ecu_variant_detection_concluded(ecu_name).await?.read().await.sdgs(service).await
    }

    async fn get_comparams(&self, ecu: &str) -> Result<ComplexComParamValue, DiagServiceError> {
        self.uds_ecu_variant_detection_concluded(ecu).await?.read().await.comparams()
    }
    ...
    async fn get_components_data_info(
        ...
        let items = self
            .uds_ecu_variant_detection_concluded(ecu).await?
            .read()
            .await
    ...
    async fn get_components_configuration_info(
        ...
        self.uds_ecu_variant_detection_concluded(ecu).await?
            .read()
            .await
    ...
    async fn get_ecu_reset_services(
        ...
        let diag_manager = self.uds_ecu_variant_detection_concluded(ecu_name).await?.read().await;
