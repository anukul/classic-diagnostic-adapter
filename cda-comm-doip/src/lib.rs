...
pub(crate) struct DoipGatewayState<T: EcuAddresses + DoipComParams> {
    ...
    /// `None` until the first successful `start()` binds it. Deliberately lazy
    /// (see `TODO(persistence-init-mode)` in `DoipDiagGateway::new`):
    /// No UDP socket is created at construction time, only when an authorized
...
pub struct DoipDiagGateway<T: EcuAddresses + DoipComParams> {
    ...
    lifecycle: Arc<DoipGatewayLifecycle>,
}

/// State needed to stop and restart all `DoIP` communication.
struct DoipGatewayLifecycle {
    /// Serializes lifecycle transitions and protects their task handles.
    operation: Mutex<DoipGatewayOperation>,
    /// Gateway-local lifecycle state. When this gateway is embedded inside a
    /// [`DiagnosticTransportRouter`], this tracker is an internal detail - the
    /// router's own tracker is authoritative for external consumers.
    /// This tracker still serves the gateway's `enable()` idempotency guard.
    coordinator: TransportStateTracker,
}
...
impl<T: EcuAddresses + DoipComParams> DoipDiagGateway<T> {
    // TODO(persistence-init-mode): `doip_socket` still lets a caller pass in
    // an already-bound socket (reused across a runtime-update reload). Once a
    // persisted topology exists, per-gateway reconnect may want to seed this
    // eagerly too; until then every fresh (non-reload) construction passes an
    // empty cell and the bind happens lazily in `start()`. See ADR-006.
    /// Create a new `DoipDiagGateway` instance.
    ///
    /// Purely in-memory: this does **not** create or bind a UDP socket, send
    /// any packet, or start any task. It is safe to call regardless of
    /// `init_mode`/authorization state.
    ///
    /// # Errors
    /// Returns `String` if initialization fails, e.g. when the configured
    /// tester address/subnet cannot be parsed into a netmask.
    #[tracing::instrument(
        skip(doip_config, ecus, variant_detection, connectivity_handler, doip_socket),
        fields(
    ...
    pub async fn new(
        ...
        variant_detection: mpsc::Sender<Vec<String>>,
        connectivity_handler: Arc<dyn EcuConnectivityHandler>,
        doip_socket: Arc<Mutex<Option<DoIPUdpSocket>>>,
        ...
        Ok(Self {
            ...
            lifecycle: Arc::new(DoipGatewayLifecycle {
                operation: Mutex::new(DoipGatewayOperation::default()),
                coordinator: TransportStateTracker::new(TransportState::Disabled),
            }),
        })
    }
    ...
    async fn start(
        ...
        // created and bound, reached only from an authorized `enable()`. A
        // reload-supplied `doip_socket` cell may already hold one (reused
        // across generations), in which case this is a no-op.
        let mut socket_guard = self.state.socket.lock().await;
        if socket_guard.is_none() {
            *socket_guard = Some(create_udp_vir_socket(
                &config.tester_address,
                config.gateway_port,
            )?);
        }
        // Just ensured `Some` above; log and fail cleanly rather than panic
        // if that invariant is ever violated instead of assuming it.
        let Some(socket) = socket_guard.as_mut() else {
            return Err(DoipGatewaySetupError::ResourceError(
                "DoIP socket unexpectedly unbound after ensuring it was set".to_owned(),
            ));
        };
...
impl<T: EcuAddresses + DoipComParams> TransportControl for DoipDiagGateway<T> {
    async fn enable(&self) -> Result<(), CommControlError> {
        ...
        // Cleanup any residual ressources, usually should be no-op
...
mod tests {
    ...
    use cda_interfaces::{
        ...
        TransportResponse, UDS_ID_RESPONSE_BITMASK, communication_control::TransportState, nrc,
        service_ids,
    };
    use doip_definitions::{
        header::ProtocolVersion,
    ...
    use crate::{
        ...
        DoipGatewayLifecycle, DoipGatewayOperation, DoipGatewayState, TransportStateTracker,
        read_ecu_responses, wait_for_ack_or_response_until_timeout,
    };
    ...
    fn make_gateway(
        ...
        DoipDiagGateway {
            ...
            lifecycle: Arc::new(DoipGatewayLifecycle {
                operation: Mutex::new(DoipGatewayOperation::default()),
                coordinator: TransportStateTracker::new(TransportState::Enabled),
            }),
        }
    }
    ...
    async fn new_never_binds_the_doip_socket() {
        let socket_cell = Arc::new(Mutex::new(None));
        ...
        assert!(
            socket_cell.lock().await.is_none(),
